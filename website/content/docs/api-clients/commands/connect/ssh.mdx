...
The Boundary connect SSH helper will automatically execute `ssh` for you,
filling in the local address and port. The Boundary connect helper SSH will
automatically accept host SSH key prompts for you and also ensures that,
regardless of your `-style` choice, will provide information to the
client in the format that makes sense: respecting the syntax and the
order of arguments.
...
### Boundary connect SSH helper without any options:
```shell-session
...
to the executed client.

```shell-session
$ boundary connect ssh -target-id ttcp_eTcZMueUYv -- -l some-other-user
```

or by passing a specific user and a local identity key file

```shell-session
$ boundary connect ssh -target-id ttcp_eTcZMueUYv -- -l myuser -i ~/.ssh/identity
```

### Boundary connect SSH helper with additional `style` flags and options:
```shell-session
$  boundary connect ssh -style=putty -target-id ttcp_eTcZMueUYv -- -l myuser -i ~/.ssh/identity
```
...
```shell-session
Usage: boundary connect ssh [options] [args]
